...
def save_diagnostic_pair(data, r_grid, theta_grid, B_lim, err_min, err_max, prefix):
    ...
    norm = colors.SymLogNorm(linthresh=1e-2, vmin=-B_lim, vmax=B_lim, base=10)
...
def run_simulation(Nphi=4, Ntheta=32, Nr=32, make_plots=False):
    ...
    R_star, target_ratio = 1.0, 0.0062
    ...
    problem.add_equation("dt(A) - eta * lap(A) + grad(phi_g) + lift(tau_A) = cross(v, curl(A))")
    ...
    cfl = de.CFL(solver, initial_dt=1e-5, cadence=10, safety=0.2, max_dt=5e-4)
    cfl.add_velocity(v)
    ...
    logger.info(f"Starting simulation: Nphi={Nphi}, Ntheta={Ntheta}, Nr={Nr}")
    ...
    while solver.proceed:
        ...
        if solver.iteration % 20 == 0:
            ...
            logger.info(f"Iter: {solver.iteration:05d} | Time: {solver.sim_time:.5f} | dt: {dt:.2e} | B_tor/B_pol: {ratio:.4f}")
            ...
            if solver.iteration % 10 == 0:
                ...
                if solver.iteration % 10 == 0:
                    logger.info(f"Iter: {solver.iteration:05d} | Time: {solver.sim_time:.4f} | Ratio: {ratio_val:.4f} | Rel Change: {rel_change:.2e}")

                if solver.sim_time > min_sim_time:
                    rel_change = np.abs(B_tor_rms - prev_B_tor) / (B_tor_rms + 1e-16)
                    
                    if rel_change < steady_state_tolerance:
                        logger.info(f"Equilibrium reached at t={solver.sim_time:.4f} (rel_change={rel_change:.2e})")
                        break
...
if __name__ == "__main__":
    ...
    run_simulation(Nphi=4, Ntheta=64, Nr=128, make_plots=True)
